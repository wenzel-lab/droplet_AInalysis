...
def get_dimensions(results, pixel_ratio, unit):
    ...
    # Use correct units: diameter in micrometers (unit), volume in picoliters (pL)
    # Pass both units to ImageData as a tuple: (diameter_unit, volume_unit)
    return ImageData(format(len(widths_vector)), (img_width, img_height),
                     ...
                     format(volume_interval), (unit, 'pL'))